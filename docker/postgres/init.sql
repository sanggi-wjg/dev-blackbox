...
COMMENT ON COLUMN task.id IS '태스크 ID';
COMMENT ON COLUMN task.user_id IS '사용자 FK';
COMMENT ON COLUMN task.title IS '태스크 제목';
COMMENT ON COLUMN task.content IS '태스크 내용';
COMMENT ON COLUMN task.tags IS '태그 (쉼표 구분 문자열)';
