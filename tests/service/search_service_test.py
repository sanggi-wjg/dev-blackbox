...
class SearchServiceTest:
    ...
    @patch("dev_blackbox.service.search_service.get_embedding_agent")
    def test_search_platform_work_logs_중복_제거(
        self,
        mock_get_agent: MagicMock,
        db_session: Session,
        user_fixture: Callable[..., User],
    ):
        # given — 같은 work_log에 여러 청크
        user = user_fixture()
        embedding = [1.0] + [0.0] * 1023
        log = PlatformWorkLog.create(
            user_id=user.id,
        ...
        service = SearchService(db_session)
        query = SearchQuery(user_id=user.id, query_text="검색어", limit=10, similarity=0.0)

        # when
        results = service.search_platform_work_logs(query)

        # then — 같은 work_log는 한 번만 반환
        assert len(results) == 1
        assert results[0].platform_work_log.id == log.id
